...
_WINDLL_FUNC int tcpmain( struct TcpdaemonServerEnvironment *p_env , int sock , void *p_addr )
{
	...
	switch( TDGetIoMultiplexEvent(p_env) )
	{
		...
		case IOMP_ON_ACCEPTING_SOCKET :
			...
			memset( & (p_accepted_session->tdhb_context) , 0x00 , sizeof(struct TDHBContext) );
		...
		case IOMP_ON_RECEIVING_SOCKET :
			...
			nret = TDHBReceiveDataWithNonblock( p_accepted_session->sock , PREHEAD_LEN , p_accepted_session->tdhb_buffer , & (p_accepted_session->tdhb_len) , & (p_accepted_session->tdhb_context) ) ;
		...
		case IOMP_ON_SENDING_SOCKET :
			...
			/* 非堵塞发送通讯数据 */
			nret = TDHBSendDataWithNonblock( p_accepted_session->sock , PREHEAD_LEN , p_accepted_session->tdhb_buffer , & (p_accepted_session->tdhb_len) , & (p_accepted_session->tdhb_context) ) ;
